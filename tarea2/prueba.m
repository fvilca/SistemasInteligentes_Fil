%A = struct('var', [1 2], 'card', [3 2], 'val', [0.5 0.8 0.1 0 0.3 0.9 ]);
%B = struct('var', [2 3], 'card', [2 2], 'val', [.5 .7 .1 .2]);
...
function C = FactorProduct(A, B)
    ...
    %dummy es cual variable
    %iA es su indice en A
    %iB es  su indice en B
    ...
    mapA = ismember(A.var, C.var); %[dummy, mapA] = ismember(A.var, C.var);
    ...
    % matriz con todas las combinaciones
    %assignments = IndexToAssignment(1:12, [3 2 2]);
    assignments = IndexToAssignment(1:prod(C.card), C.card);
    
    %columna con todos los valores en A que se repiten en el assigment
    %indxA = AssignmentToIndex(assignments(:, [2 1 3]), [3 2]);
